...
- **视频**：MiniMax H3、BytePlus Seedance 2.x 与旧 Hailuo 分别走真实供应商协议；支持原生音视频、参考音频驱动和后期配音。
...
- **口型验收**：有可见对白的真实视频必须标记口型已通过才能合并成片；单纯把 TTS 混入 MP4 不视为口型同步。
- **成本**：只有供应商返回真实费用时才记录金额；缺少账单数据时显示“成本不完整”，不会把未知费用当成零成本。

模型能力、适用镜头与外部配音取舍见 [原生音视频与外部配音调研](./docs/native-audio-external-dubbing-research.md)。
