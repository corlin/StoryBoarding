  ...
  return (
    <>
      <div
        className={
          compact
            ? "flex items-center gap-2 max-w-full overflow-x-auto no-scrollbar"
            : "flex items-center justify-between px-6 py-2.5 bg-[#121316]/90 border-b border-border/40 backdrop-blur-sm"
        }
      >
        {/* Left: Episode Selector Pills & Add Episode Button */}
        <div className="flex items-center gap-1.5 overflow-x-auto no-scrollbar py-0.5">
          <div className="flex items-center gap-1 text-[11px] font-mono text-muted-foreground mr-0.5 shrink-0">
            <Film className="w-3.5 h-3.5 text-amber-400" />
            <span className="hidden sm:inline">短剧分集:</span>
          </div>
          ...
          <button
            ...
            className={`flex items-center gap-1 ${
              compact ? "px-2 py-0.5 text-[11px]" : "px-3 py-1.5 text-xs"
            } bg-primary/10 hover:bg-primary/20 border border-primary/30 text-primary font-medium rounded-md transition shrink-0 shadow-2xs`}
            title="在当前工程内无缝续写并追加下一集（继承全剧角色视觉 DNA）"
          >
            <Plus className="w-3 h-3" />
            <span className="hidden sm:inline">追加集</span>
          </button>
        ...
        {/* Right: Character Hub Drawer Trigger (Hidden in compact mode since TopBar has Bible) */}
        {!compact && (
          <div className="flex items-center gap-3 shrink-0 ml-4">
            ...
            >
              ...
              <span className="font-mono text-[10px] bg-muted px-1.5 py-0.2 rounded text-foreground">
                {characters.length}
              </span>
            </button>
          </div>
