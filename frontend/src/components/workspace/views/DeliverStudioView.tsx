...
import { exportScreenplayToMarkdown, generateScreenplayMarkdownContent } from "@/lib/screenplayMarkdownExporter";
...
import ProductionKanbanView from "@/components/workspace/kanban/ProductionKanbanView";
...
export const DeliverStudioView: React.FC<DeliverStudioViewProps> = ({
  ...
  const [activeSubTab, setActiveSubTab] = useState<"deliverables" | "kanban">("deliverables");
  ...
  // 2b. Export Screenplay & Dramatic Production Call Sheet (Markdown)
  const handleExportMarkdown = () => {
    if (!project) return;
    try {
      const sequences = project.sequences || [];
      exportScreenplayToMarkdown(project, sequences, {
        scope: exportScope,
        currentSequenceId: shots[0]?.sequence_id || sequences[0]?.id,
      });
      notify.success("✅ 编剧手记与剧作交接通告台本 (Markdown) 已下载！");
    } catch (err: any) {
      notify.error(`Markdown 导出失败: ${err.message || "请稍后重试"}`);
    }
  };
  ...
  const handleExportZip = async () => {
    ...
    try {
      ...
      // 1b. Screenplay & Dramatic Production Call Sheet (Markdown)
      setZipProgressText("正在编排编剧手记与剧作交接通告台本 (Markdown)...");
      try {
        const screenplayMd = generateScreenplayMarkdownContent(
          project,
          project.sequences || [],
          { scope: exportScope, currentSequenceId: shots[0]?.sequence_id }
        );
        rootFolder?.file("00_编剧手记与剧作交接通告台本.md", screenplayMd);
      } catch (mdErr) {
        console.warn("Skip Markdown generation in zip:", mdErr);
      }
      ...
      // 4. Lossless Project JSON
      setZipProgressText("正在写入工程元数据与分镜索引...");
      ...
      rootFolder?.file("04_工程分镜完整数据.json", JSON.stringify(masterMetadata, null, 2));

      // 5. Readme / Delivery Manifest
      const manifest = `# ${project.title} · 分镜工程交付说明 (Delivery Manifest)
导出时间: ${new Date().toLocaleString()}
总镜头数: ${activeShots.length} 镜
包含内容:
0. 00_AI视频生成参考图集_含机位运镜标识/
   - Shot_XXX_*.png: 每个镜头的独立关键帧图（已标注机位序号、景别、运镜方向与台词），可用作视频大模型首帧垫图与对齐基准
   - ai_video_generation_prompts.json: 结构化 AI 视频生成提示词清单（含中英双语提示词、运镜、景别、时长与负向提示词），方便脚本与自动化管线直接读取
   - ai_video_control_rig.csv: 表格版控制指令（兼容 Excel / pandas / Google Sheets），包含生成视频所需的参数字段
1. 01_分镜打样表.png - 高清景别/运镜/对白排版长图 (Previz Sheet)
2. 02_剧组顺场排期表.csv - 场景空间、光影氛围与出场角色统计表
3. 03_AI视频提示词工程包/ - 适配 MiniMax 海螺 H3、SeaDance 2.5、Wan 2.1、Runway/可灵 的机位提示词
4. 04_工程分镜完整数据.json - 包含剧本、角色卡、场景设定与分镜时码的完整归档数据

Generated by StoryBoarding AI Studio.`;
      rootFolder?.file("README_交付说明.txt", manifest);

      // Generate ZIP and trigger instant download
      setZipProgressText("正在压缩打包全套工程资料...");
      ...
      a.href = url;
      a.download = `${safeTitle}_分镜工程包_${dateStr}.zip`;
      document.body.appendChild(a);
      a.click();
      document.body.removeChild(a);
      URL.revokeObjectURL(url);

      notify.success("🎉 全套分镜工程资料包已打包完成并开始下载！");
    } catch (err: any) {
      console.error(err);
      notify.error(`打包失败: ${err.message || "请稍后重试"}`);
    } finally {
    ...
    <div className="flex-1 flex flex-col h-full bg-background overflow-hidden">
          ...
          <div>
            <div className="flex items-center gap-2">
              <h2 className="text-xs md:text-sm font-bold text-foreground">
                STAGE 04 · 导出与生产协作中心 (Deliver Suite)
              </h2>
            ...
            <p className="text-[11px] text-muted-foreground hidden sm:block">
              导出分镜长图、制片顺场表、视频模型提示词包与全套工程归档
            </p>
          </div>
        </div>

        <div className="flex items-center gap-2">
          {/* Subtabs Toggle: Deliverables Suite vs Production Kanban */}
          <div className="flex items-center gap-1 bg-secondary/80 p-0.5 rounded-lg border border-border text-xs">
            <button
              type="button"
            ...
            <button
              ...
              className={cn(
                "px-2.5 py-1 rounded-md transition-all font-medium cursor-pointer flex items-center gap-1.5",
                activeSubTab === "kanban"
                  ? "bg-card text-emerald-300 font-bold shadow-xs border border-border/50"
                  : "text-muted-foreground hover:text-foreground"
              )}
            >
      ...
      {activeSubTab === "kanban" ? (
        <ProductionKanbanView
          ...
          onBackToDeliverables={() => setActiveSubTab("deliverables")}
        />
      ) : (
        ...
        <div className="flex-1 lg:w-2/3 overflow-y-auto p-4 md:p-6 space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {/* Card 0: Screenplay & Dramatic Production Call Sheet (Markdown) */}
            <div className="p-4 rounded-xl bg-card border border-border/80 hover:border-amber-500/50 transition-all shadow-xs space-y-3 flex flex-col justify-between">
              <div className="space-y-2">
                ...
                <div>
                  <h4 className="text-sm font-bold text-foreground">编剧手记与交接通告表 (Screenplay Call Sheet)</h4>
                  <p className="text-xs text-muted-foreground mt-1 leading-relaxed">
                    涵盖系列世界观圣经、梅峰价值转折、斯奈德对撞与好莱坞视听隐喻信物微动作，适于 Obsidian、飞书与剧组交接。
                  </p>
                </div>
              </div>
              ...
              >
                ...
                <span>导出编剧手记与交接台本 (Markdown)</span>
              </button>
            </div>
            ...
            <div className="p-4 rounded-xl bg-card border border-border/80 hover:border-primary/50 transition-all shadow-xs space-y-3 flex flex-col justify-between">
              <div className="space-y-2">
                ...
                <div>
                  <h4 className="text-sm font-bold text-foreground">全套分镜工程资料包 (Project Archive)</h4>
                  <p className="text-xs text-muted-foreground mt-1 leading-relaxed">
                    打包下载分镜长图 (PNG)、顺场表 (CSV)、主流模型提示词包 (TXT) 与工程数据 (JSON)。
                  </p>
                </div>
                {zipProgressText && (
              ...
              >
                ...
                <span>{isExportingZip ? "正在打包工程文件..." : "打包下载全套资料 (ZIP)"}</span>
              </button>
            </div>
          </div>
        </div>
        ...
        <div className="flex-1 lg:w-1/3 border-t lg:border-t-0 lg:border-l border-border/70 bg-card/30 p-4 md:p-6 space-y-5 overflow-y-auto">
          ...
          <div className="rounded-xl border border-emerald-500/30 bg-emerald-500/5 p-3 space-y-2" role="status">
            <p className="text-xs text-emerald-300">
              本区导出分镜打样长图、制片通告单和提示词；完整成片 MP4、分集配音与字幕可直达生产看板查看。
            </p>
            <button
              type="button"
              onClick={() => setActiveSubTab("kanban")}
              className="inline-flex items-center gap-1.5 rounded-lg bg-emerald-600 px-3 py-2 text-xs font-semibold text-white transition-colors hover:bg-emerald-500 cursor-pointer shadow-xs"
            >
              <Film className="h-3.5 w-3.5" />
              <span>切换至实拍与成片看板</span>
