...
【核心视觉导演六大铁律 (CRITICAL DIRECTING PRINCIPLES)】:
1. 【晚进早出与剪辑切口 (Enter Late, Leave Early · 决不拍过渡垃圾镜头)】:
   - 每一个镜头必须在事件/冲突爆发前的最后 1 秒直接入画，在情绪顶点或动作未完待续瞬间切出！
   - 严禁任何形式的进门、倒水、走路、坐下等枯燥生活日常交代；首镜直插危机焦点！

2. 【镜头微观价值转折 (Micro Value Turn · 杜绝静态废镜)】:
   - 镜头不是死板插画，而是叙事推动器。每个关键镜头必须使角色的【权力位置、情绪极性或信息知晓度】发生微位移（如从“胜券在握”转向“突遭背刺”，从“隐忍退让”转向“绝地爆发”）；
   - 没有带来状态改变或悬念加深的镜头严禁生成。

3. 【对白即行动与潜台词 (Dialogue is Action · 严禁画外说明书)】:
   - 台词必须是行动的武器（攻击、试探、掩盖、反制），一句台词只办一件事！
   - 坚决杜绝解释性台词与画外音自我介绍（如“我叫XXX”）；解说必须转化为交锋时的致命弹药；
   - 追求冰山下的“未说之话 (Subtext)”，表面平静克制，台词背后是涌动的杀机或深情；无人说话写“无”。
...
5. 【特鲁比四角对立与系列引擎潜意识驱动 (Truby Opposition & Series Engine Driven)】:
   - 若上下文中提供了【系列引擎】或【角色四角对立定位】：
     * 对手 (main_opponent) 的交锋台词必须精准刺向主角的致命软肋 (attacks_flaw)，绝不允许说不痛不痒的口水话；
     * 假盟友 (fake_ally) 的台词必须暗藏试探与双重语义，表面站队实则探询底牌；
     * 动作调度严格呼应【默契契约底线】与【对立统一纽带】，优先调度承重信物 (bonding_items) 或利用空间权力差展示心理博弈。
