      ...
      seqs.map(async (seq) => {
        ...
        return {
          ...
          shots: shotList.map((s) => ({
            ...
            visual_metaphor: s.continuityData ? JSON.parse(s.continuityData)?.visual_metaphor : undefined,
      ...
      for (const s of epShots) {
        ...
        await db.insert(shots).values({
          ...
          continuityData: JSON.stringify({
            ...(s.continuity_data || {}),
            visual_metaphor: s.visual_metaphor || (s.continuity_data as any)?.visual_metaphor,
          }),
          beatType: s.beat_type || "tension_build",
          emotionalVoltage: Number(s.emotional_voltage) || 50.0,
          informationGap: s.information_gap || "",
