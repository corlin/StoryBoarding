...
const videoRouteSource = fs.readFileSync(path.join(__dirname, '../src/routes/video.ts'), 'utf8');
...
test('legacy AI-generated TTS references recover trusted consent and R2 file metadata', () => {
  assert.match(videoRouteSource, /source === "ai_generated"/);
  assert.match(videoRouteSource, /voiceConsentStatus: "provider_preset"/);
  assert.match(videoRouteSource, /STORAGE\.head/);
});
